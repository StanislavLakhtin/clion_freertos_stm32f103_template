...
// Проинициализированный USB будет иметь true
...
void cdcacm_data_rx_cb( usbd_device * usbd_dev, uint8_t ep ) {
  ...
  // Проверка достаточной глубины очереди
  unsigned rx_avail = uxQueueSpacesAvailable( usb_rxq );
  char buf[64];       // Это RX буфер. Читаем либо по 64 char, либо на всю длинну очереди, если она меньше
  if ( rx_avail <= 0 ) {
    gpio_clear(GPIOC, GPIO13); // todo save error state
    return;
  }

  int len = sizeof( buf ) < rx_avail ? sizeof buf : rx_avail;
...
int usb_getline( char * buf, unsigned buf_size ) {
  return getline( buf, buf_size, usb_getc, _putchar );
}

void usb_processor( void * arg ) {
  ...
  do {
    ...
    if ( initialized ) {
      while ( txlen < sizeof txbuf && xQueueReceive( usb_txq, &txbuf[ txlen ], 0 ) == pdPASS)
        ++txlen;        /* Сдвигаем указатель на количество принятых сообщений */
      if ( txlen > 0 ) {
        if ( usbd_ep_write_packet( udev, 0x82, txbuf, txlen ) != 0 )
          txlen = 0;      /* Сбрасываем счётчик, если всё ok */
      } else {
        taskYIELD();        /* Молотим дальше CPU */
      }
    } else {
      taskYIELD(); // todo моргать светодиодом неистово об ошибке
    }
  } while ( true );
}
...
usbd_device * usb_setup( void ) {
  ...
  usb_txq = xQueueCreate( TX_RX_QUEUE_LENGTH, sizeof( char ));
  usb_rxq = xQueueCreate( TX_RX_QUEUE_LENGTH, sizeof( char ));
